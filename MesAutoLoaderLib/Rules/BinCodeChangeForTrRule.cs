...
{
    class BinCodeChangeForTrRule : IBinCodeChangeRule
    {
        Dictionary<string, string> binNoChangeMap;
        Dictionary<string, decimal> binCodeQtyMap;

        public BinCodeChangeForTrRule(Dictionary<string, string> binNoChangeMap, Dictionary<string, decimal> binCodeQtyMap)
        {
            ...
            foreach (DataRow binRow in mapDataTb.Rows)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        binRow["Bin"] = "100";
                        binRow["BIN_CODE"] = "NotG-Bin";
                    }
                }

                if (binCodeQtyMap.ContainsKey(binCode))
                {
                    if (binCodeQtyMap[binCode] < 30)
                    {
                        binRow["Bin"] = "100";
                        binRow["BIN_CODE"] = "NotG-Bin";
                    }
